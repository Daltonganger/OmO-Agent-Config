...
function parseModels(output) {
  ...
  for (const line of lines) {
    // Match model IDs like "provider/model" or "provider/path/model" (supports Chutes multi-path format)
    if (line.match(/^[a-zA-Z0-9-]+(?:\/[a-zA-Z0-9-._]+)+$/i) && braceCount === 0) {
...
function scoreModel(model, agentType, config) {
  ...
  for (const pref of profile.preferred) {
    switch (pref) {
      ...
      case "deep_work":
        if (caps.coding || model.capabilities?.coding) {
          score += 15;
        } else if (caps.autonomous || model.capabilities?.autonomous) {
          score += 12;
        } else if (caps.reasoning && context >= 128000) {
          score += 10;
        }
        break;
    }
  ...
  const releaseDate = model.releaseDate || model.released;
  if (releaseDate && typeof releaseDate === 'string') {
    try {
      const release = new Date(releaseDate);
      if (!isNaN(release.getTime())) {
        const now = new Date();
        const ageInDays = (now - release) / (1000 * 60 * 60 * 24);
      ...
      // Invalid date format, skip recency scoring
    }
  }
