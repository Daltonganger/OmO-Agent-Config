...
const { colors, CONFIG_DIR, BACKUP_DIR, CONFIGS_DIR, CACHE_DIR, SECRETS_DIR, OPENCODE_CONFIG_FILE, DEFAULTS, AGENT_PROFILES, CONFIG_FILE, AGENT_PRIORITY_ORDER, AGENT_MODEL_REQUIREMENTS, CATEGORY_MODEL_REQUIREMENTS, ALL_CATEGORIES } = require('../constants');
const { ConfigurationManager } = require('../config-manager');
const { loadModels, getRecommendedModels, hasExtendedThinking, isFastModel, scoreModel } = require('../model-loader');
...
const { resolveModelPipeline, validateAgentModel, validateCategoryModel } = require('../model-resolution');
const { isValidCategory, listAllCategories } = require('../category-loader');
  ...
  async displayModelList(filtered, agentType, currentModel) {
    ...
    while (true) {
      ...
      for (let i = start; i < end; i++) {
        ...
        const modelScore = scoreModel(filtered[i], agentType, this.config);
        console.log(`  ${i + 1}. ${formatModel(filtered[i], true, modelScore)}${current}`);
      }
  ...
  async agentConfigMenu() {
    while (true) {
      ...
      const agents = Object.entries(this.config.agents || {})
        .sort(([a], [b]) => {
          const idxA = AGENT_PRIORITY_ORDER.indexOf(a);
          const idxB = AGENT_PRIORITY_ORDER.indexOf(b);
...
OPTIONS:
  ...
  -s, --switch <name>     Switch to specified configuration
      ...
      --resolve <name>    Resolve model for agent/category
...
EXAMPLES:
  ...
  opencode-agent-config --validate         # Validate current config
  opencode-agent-config --resolve oracle   # Resolve model for oracle
`);
  }
  ...
  async validateConfigCli() {
    ...
    if (!this.models) {
      try {
        ...
        console.error('Warning: Could not load models from OpenCode');
        this.models = [];
        this.providers = [];
      }
    }
    
    const availableModelIds = this.models.map(m => m.id);
    
    for (const agentName of Object.keys(this.config.agents || {})) {
      const result = validateAgentModel(agentName, availableModelIds, this.config);
      if (!result.valid) {
    ...
    for (const categoryName of Object.keys(this.config.categories || {})) {
      const result = validateCategoryModel(categoryName, availableModelIds, this.config);
      if (!result.valid) {
        errors.push(`Category "${categoryName}": ${result.error}`);
      }
    }
  ...
  async resolveModelCli(agentOrCategory, variant) {
    let retries = 3;
    let loaded = null;
    ...
    while (retries > 0 && !loaded) {
      try {
        ...
        retries--;
        if (retries === 0) {
          console.error('Error: Could not load models from OpenCode after 3 attempts');
          process.exit(1);
        }
    ...
    const result = resolveModelPipeline(availableModelIds, this.config, agentOrCategory);
    
    if (!result) {
      console.error(`Error: Could not resolve model for "${agentOrCategory}"`);
      process.exit(1);
    }
  ...
  async handleCliArgs(args) {
    ...
    switch (command) {
      ...
      case '--validate':
        await this.loadConfig();
        await this.validateConfigCli();
        process.exit(0);
        break;

      case '--resolve':
        if (!args[1]) {
          console.error('Error: Agent or category name required');
          console.log('Usage: opencode-agent-config --resolve <agent-or-category> [--variant <variant>]');
          process.exit(1);
        }
        await this.loadConfig();
        const variant = args.includes('--variant') ? args[args.indexOf('--variant') + 1] : null;
        await this.resolveModelCli(args[1], variant);
        process.exit(0);
        break;
